...
## 🤔 Edge Case
- [X] 프로모션 재고가 부족하여 일부 수량을 프로모션 혜택 없이 결제해야 하지만, 일부 수량도 없을 경우
- [X] 멤버십 할인이 8000원이 넘어가는 경우
- [X] 오늘 날짜가 프로모션 기간이 아닌 경우 -> 일반 수량이랑 합쳐야하는 지 고민
- [X] 프로모션이 존재하는 상품이지만, 사용자가 지정된 개수보다 적개 구매할 경우
- [X] 고객이 구매한 상품의 수량 중 일부만 프로모션 재고에 해당되는 경우
- [X] 프로모션이 종료되는 날의 특정 시간에 고객이 구매를 시도하는 경우 -> 0시를 기준으로 할지 생각
- [X] 제품명이 너무 긴 경우
- [X] 수량이 너무 긴 경우 -> 1,000 표시할 것인가?
- [X] 금액이 너무 긴 경우
